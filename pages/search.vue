<template>
	<view class="content">
		<view class="search-box">
			<mSearch class="mSearch-input-box" :mode="2" button="inside" :placeholder="defaultKeyword" @search="doSearch(false)" @input="inputChange" @confirm="doSearch(false)" v-model="keyword" @showKeyWordList="showKeyWordList" ref="child"></mSearch>
		</view>
		<view class="search-keyword" @touchstart="blur" v-if="!isShowProductList">
	...
	export default {
		...
		methods: {
			...
			loadHotKeyword() {
				...
				this.hotKeywordList = ['牙膏', '洗发水', '麦片', '卡丽施', '爱生活'];
			},
			// 判断关键词搜索列表
			showKeyWordList () {
